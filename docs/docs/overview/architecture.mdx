...
<DocPageHeader title="Architecture Overview" />
...
* Client-side App
* Client-side Command-line Interface (CLI)
* Server-side API
* Server-side Worker

For more information on inputs that each component supports, see the [Environment Variables](../deploy/environment-variables.mdx) section of the deployment docs.
...
It's limited in functionality today, but currently gives the ability to do key operations such as triggering a workflow, and synchronizing a conneciton down to a locally hosted database.
...
It is backed by a Postgres Database, where it stores all of its configured, save for explicit workflow data, which is deferred to Temporal.
...
[Tempora](https://temporal.io/) is the primary workflow engine that Neosync uses to invoke all of the Neosync Jobs.
