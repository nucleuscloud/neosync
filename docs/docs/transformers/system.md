...
| [Scramble Identity](/transformers/system#transform-scramble-identity)             | int64   | Designed for auto-incrementing columns. Takes an input value and scrambles it (ensures the output is not the same). This transformer handles UNIQUE and Primary Key auto-incrementing values. |
...
### Transform Scramble Identity\{#transform-scramble-identity}

Anonymizes and transforms integers in a unique way. Designed for auto-incrementing primary key values. The input will never equal the output.

**Configurations**

There are no configurations for this transformer at this time.
