...
func New(t testing.TB, opts ...Option) *slog.Logger {
	...
	go func() {
		for msg := range h.logCh { // Will exit when channel is closed
			t.Log(msg)
		}
	}()
...
func (b *Bridge) Handle(ctx context.Context, rec slog.Record) error { //nolint:gocritic
	b.mu.Lock()
	defer b.mu.Unlock()
