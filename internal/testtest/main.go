	...
	for _, stmt := range tree.GetStmts() {
		...
		for _, col := range s.GetCreateStmt().GetTableElts() {
			if col.GetColumnDef() != nil {
				...
				// Debug output
				jsonBytes, _ := json.MarshalIndent(colDef, "", "  ")
				fmt.Printf("Column %s definition:\n%s\n", colDef.Colname, string(jsonBytes))

				isGenerated := false
				if colDef.Identity != nil {
				...
				if colDef.DefaultVal != nil {
					fmt.Printf("DefaultVal: %v\n", colDef.DefaultVal)
					// Check if it's a serial/identity column
					if expr, ok := colDef.DefaultVal.Node.(*pg_query.Node_FuncCall); ok {
						fmt.Printf("DefaultVal is function: %v\n", expr.FuncCall)
					}
				}
		...
		case *pg_query.Node_CreateStmt:
			...
			for _, col := range s.GetCreateStmt().GetTableElts() {
				...
				fmt.Println(col.GetColumnDef().Generated)
				jsonF, _ := json.MarshalIndent(col.GetColumnDef(), "", " ")
				col.
					fmt.Printf("\n\n %s \n\n", string(jsonF))
				if col.GetColumnDef() != nil {
					isGenerated := false
					if col.GetColumnDef().GetIdentity() != "" || col.GetColumnDef().GetGenerated() != "" {
						isGenerated = true
					}
					columns = append(columns, &Column{
						Name:        col.GetColumnDef().Colname,
