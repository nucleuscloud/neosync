...
func TestNeosyncDateTime(t *testing.T) {
	t.Run("ScanPgx_Timestamp", func(t *testing.T) {
		...
		assert.Equal(t, 123456789, dt.Nano)
	...
	t.Run("ScanPgx_Timestamptz", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, 2023, dt.Year)
		assert.Equal(t, 12, dt.Month)
		assert.Equal(t, 25, dt.Day)
		assert.Equal(t, 23, dt.Hour)
		assert.Equal(t, 59, dt.Minute)
		assert.Equal(t, 59, dt.Second)
		assert.Equal(t, 999999999, dt.Nano)
	...
	t.Run("ScanMysql_BC", func(t *testing.T) {
		...
		assert.Equal(t, 100, dt.Year)
		assert.Equal(t, 7, dt.Month)
		assert.Equal(t, 4, dt.Day)
		assert.Equal(t, 16, dt.Hour)
		assert.Equal(t, 20, dt.Minute)
		assert.Equal(t, 30, dt.Second)
		assert.Equal(t, "America/New_York", dt.TimeZone)
		assert.True(t, dt.IsBC)
	})
