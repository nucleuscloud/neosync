	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for idx, a := range actual {
				assert.ElementsMatch(t, tt.expect[idx], a)
			}
		})
	}
}
...
func Test_GetSyncConfigs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expect: []*SyncConfig{
				{Table: "public.a", Columns: &SyncColumn{Exclude: []string{"a_id"}}},
		...
		},
		{
			...
			tables: []string{"public.a"},
			expect: []*SyncConfig{
				{Table: "public.a", Columns: &SyncColumn{Exclude: []string{"a_id", "aa_id"}}},
		...
		},
		{
			...
			expect: []*SyncConfig{
				{Table: "public.a", Columns: &SyncColumn{Exclude: []string{"b_id"}}},
		...
		},
		{
			...
			tables: []string{"public.a", "public.b", "public.c", "public.d", "public.e"},
			expect: []*SyncConfig{
				{Table: "public.b", Columns: &SyncColumn{Exclude: []string{"c_id", "d_id"}}},
				{Table: "public.e", DependsOn: []*DependsOn{{Table: "public.b", Columns: []string{"id"}}}},
