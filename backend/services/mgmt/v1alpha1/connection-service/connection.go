...
func (s *Service) GetConnections(
	...
	if err != nil && !errors.IsNotFound(err) {
		logger.Error(fmt.Errorf("unable to retrieve connections: %w", err).Error())
		return nil, err
	} else if err != nil && errors.IsNotFound(err) {
		logger.Error(fmt.Errorf("connections not found: %w", err).Error())
		return nil, err
	}
...
func (s *Service) CreateConnection(
	...
	errs.Go(func() error {
		...
		if err != nil && !errors.IsAlreadyExists(err) {
			...
			if err != nil {
				logger.Error(fmt.Errorf("unable to update connection secret: %w", err).Error())
				return err
			}
		}
		select {
	...
	if err != nil && !errors.IsAlreadyExists(err) {
		...
		if deleteSecretErr != nil {
			logger.Error(fmt.Errorf("unable to clean up connection secret: %w", err).Error())
		}
		deleteConnErr := s.k8sclient.CustomResourceClient.Delete(ctx, connection, &runtimeclient.DeleteOptions{})
		if deleteConnErr != nil {
			logger.Error(fmt.Errorf("unable to clean up connection: %w", err).Error())
		}
		return nil, err
	} else if err != nil && errors.IsAlreadyExists(err) {
		return nil, err
...
func (s *Service) UpdateConnection(
	...
	if connection.Secret != nil {
		...
		if err != nil {
			logger.Error(fmt.Errorf("unable to update connection: %w", err).Error())
			return nil, err
		}
...
func (s *Service) DeleteConnection(
	...
	err = errs.Wait()
	if err != nil {
		logger.Error(fmt.Errorf("unable to delete connection: %w", err).Error())
		return nil, err
	}
...
func getConnectionSecretByName(
	...
	if err != nil && !errors.IsNotFound(err) {
		...
		logger.Error(fmt.Errorf("connection secret not found: %w", err).Error(), "secretName", name)
		return nil, err
	}
	return secret, nil
}
...
func getSqlConnectionById(
	...
	if err != nil {
		logger.Error(fmt.Errorf("unable to retrieve connection: %w", err).Error())
		return nil, err
	}
	if len(conns.Items) == 0 {
