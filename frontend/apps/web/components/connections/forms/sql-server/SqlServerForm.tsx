...
interface MssqlFormStore extends BaseStore<MssqlFormValues> {
  init?(values: MssqlFormValues): void;
}
...
const useFormStore = create<MssqlFormStore>((set) => ({
...
export default function SqlServerForm(props: Props): ReactElement {
  ...
  async function handleSubmit(e: FormEvent): Promise<void> {
    ...
    try {
      ...
      const validatedData = await MssqlFormValues.validate(formData, {
        abortEarly: false,
        context: {
        ...
        <CheckConnectionButton
          ...
          getRequest={() => {
            return createMessage(CheckConnectionConfigRequestSchema, {
              connectionConfig: buildConnectionConfigMssql({
                ...formData,
