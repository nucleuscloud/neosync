...
func Test_GenerateRandomIntPositive(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		res, err := GenerateRandomInt64(false, min, max, 0, maxCharacterLimit)
...
func Test_GenerateRandomIntMaxLength(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		res, err := GenerateRandomInt64(false, min, max, 0, 3)
		assert.NoError(t, err)
...
// func Test_RandomizeSign(t *testing.T){
