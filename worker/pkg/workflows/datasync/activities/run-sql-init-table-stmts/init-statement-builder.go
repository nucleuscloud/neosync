...
import (
	...
	destinationdatabasebuilder "github.com/nucleuscloud/neosync/internal/destination-database-builder"
...
func (b *initStatementBuilder) RunSqlInitTableStatements(
	...
	for _, destination := range job.Destinations {
		...
		if shouldInitSchema {
			initSchemaErrors, err := ddbuilderService.InitializeSchema(ctx, uniqueTables)
			if err != nil {
		...
		err = ddbuilderService.TruncateData(ctx, uniqueTables, uniqueSchemas)
		if err != nil {
...
type InitSchemaRunContext struct {
	ConnectionId string
	Errors       []*destdb_shared.InitSchemaError
}
