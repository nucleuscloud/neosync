...
func (s *IntegrationTestSuite) Test_Workflow_Sync() {
	tests := testdata_doublereference.GetTests()

	for _, test := range tests {
		s.T().Logf("running integration test: %s \n", test.Name)
		s.SetupSourceDb(test.Folder, test.SourceFilePaths)
		s.SetupTargetDb(test.Folder, test.TargetFilePaths)

		schemas := []*mgmtv1alpha1.PostgresSourceSchemaOption{}
		subsetMap := map[string]*mgmtv1alpha1.PostgresSourceSchemaOption{}
		for table, where := range test.SubsetMap {
		...
		for table, expected := range test.Expected {
			...
			require.Equalf(s.T(), expected.RowCount, count, fmt.Sprintf("Test: %s Table: %s", test.Name, table))
		}

		s.TearDownTestByFolder(test.Folder)
	}
}
