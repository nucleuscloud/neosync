...
func NewGenerateFullNameOptsFromConfig(
	config *mgmtv1alpha1.GenerateFullName,
	maxLength *int64,
...
func generateRandomFullName(randomizer rng.Rand, maxLength int64) (string, error) {
	...
	if maxLengthMinusSpace <= 0 {
		return "", fmt.Errorf(
			"unable to generate full name including space with provided max length: %d",
			maxLength,
		)
	}
	...
	if maxFirstNameIdx == -1 || maxLastNameIdx == -1 {
		return "", fmt.Errorf(
			"unable to generate a full name with the provided max length: %d",
			maxLength,
		)
	}
	...
	if err != nil {
		return "", fmt.Errorf(
			"unable to generate random first name with length: %d",
			maxFirstNameLength,
		)
	}
	...
	if err != nil {
		return "", fmt.Errorf(
			"unable to generate random last name with length: %d",
			maxLastNameLength,
		)
	}
