	...
	err := bloblang.RegisterFunctionV2(
		...
		func(args *bloblang.ParsedParams) (bloblang.Function, error) {
			...
			return func() (any, error) {
				out, err := transformer_utils.GenerateRandomStringWithInclusiveBounds(
					randomizer,
					min,
					max,
				)
...
func NewGenerateRandomStringOptsFromConfig(
	config *mgmtv1alpha1.GenerateString,
	maxLen *int64,
	...
	if minValue != nil {
		newMin := transformer_utils.MinInt(
			*minValue,
			*maxValue,
		) // ensure the min is not larger than the max allowed length
		minValue = &newMin
	}
...
func (t *GenerateRandomString) Generate(opts any) (any, error) {
	...
	return transformer_utils.GenerateRandomStringWithInclusiveBounds(
