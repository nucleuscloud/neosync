...
func (m *defaultTransformerProcessor) ProcessBatch(ctx context.Context, batch service.MessageBatch) ([]service.MessageBatch, error) {
	...
	for i, msg := range batch {
		...
		// 	continue
		// }
		mutations, err := anyToAttributeValueTest("", root, m.mappedKeys, m.defaultTransformerMap, []string{})
		if err != nil {
...
func anyToAttributeValueTest(path string, root any, mappedKeys map[string]struct{}, transformerMap map[string]*mgmtv1alpha1.JobMappingTransformer, mutations []string) ([]string, error) {
	...
	case []byte:
		...
		if t.Source != mgmtv1alpha1.TransformerSource_TRANSFORMER_SOURCE_PASSTHROUGH {
			jm.Transformer = t
			mutation, err := genbenthosconfigs_activity.ComputeMutationFunction(jm, nil)
			if err != nil {
				return nil, err
			}
			mutations = append(mutations, fmt.Sprintf("%s = %s", path, mutation))
		}
