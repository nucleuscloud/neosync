...
In this guide, we're going to walk through how you can anonymize sensitive data in your RDS database with synthetic data for testing and rapid development using [Neosync](https://neosync.dev). Neosync is an open source synthetic data orchestration company that can create anonymized or synthetic data and sync it across all of your rds environments for better security, privacy and development.

If you haven't already done so, follow the [Seeding your Rs DB with Synthetic Data](/neosync-rds-data-gen-job.mdx) blog to get set up.
...
So now we have two databases: a source and a destination. I already have data in my source database but if you don't you follow our [Seeding your RDS DB with Synthetic Data](/neosync-rds-data-gen-job.mdx) blog to seed your database with data.
