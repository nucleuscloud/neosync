---
title: Using Neosync and OpenAI to generate synthetic data
description: A walkthrough tutorial on how to integrate Neosync with OpenAI to generate synthetic
date: 2024-04-04
published: true
...
We recently helped host a [hackathon at the Github HQ](https://www.linkedin.com/feed/update/urn:li:activity:7178809047574351873) and one of the projects that we enjoyed the most was a hacker integrating Neosync with OpenAI in order to generate synthetic data. As soon as we saw it, a hundred different use-cases came to mind of how this could be useful.

So I wanted to write a blog and a guide on exactly how you can integrate Neosync with not just OpenAI but any model that is available at some endpoint. We're going to use the use case that we have a customer interview platform that we want to generate synthetic data for our staging and development environments. Because the production customer interview data is sensitive, generating synthetic data is a secure and privacy-safe way of giving developers the data they need to build new features without compromising on data privacy.
...
3. A database to store your data. Either Postgres or Mysql works.
...
![new-trans](/images/blogs/cust-gen-db.png)
...
![new-trans](/images/blogs/cust-gen-db-create.png)
...
Neosync has the notion of Transformers. Transformers are modules that can generate synthetic data or anonymize existing data. We ship Neosync with 40+ transformers out of the box but you can also create your own custom tranformers in code which is what we're going to do for this use case.

Click on **Transformers** in the top navigation menu and then the **+ New Transformer** button.

![trans-page](/images/blogs/newtra.png)
...
![new-trans](/images/blogs/newcustomtrans.png)
...
![new-trans](/images/blogs/oicode.png)
...
![new-trans](/images/blogs/cust-gen-define.png)
...
![new-trans](/images/blogs/cust-gen-connect.png)
...
![new-trans](/images/blogs/base-schema.png)
...
Now is the fun part. We can open up our database and see what our data looks like. I'm usin DBeaver as a database client to connect to my Neon database but you can also just use the SQL Editor in Neon to query your database.
